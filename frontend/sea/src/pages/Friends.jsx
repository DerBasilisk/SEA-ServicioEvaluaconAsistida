        ...
        <div className="flex gap-2 mb-6 overflow-x-auto">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              {t.label}
              {t.count > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${tab === t.id ? "bg-violet-400" : "bg-indigo-700"}`}>{t.count}</span>
              )}
            </button>
          ))}
        </div>
                ...
                {leaderboard.map((entry, i) => {
                  ...
                  return (
                    <div key={entry.user._id}
                      ...
                      <Avatar src={entry.user.avatar} name={displayName} size="md" />
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
          <div className="bg-indigo-900 border border-indigo-700 rounded-2xl p-6 w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="overflow-y-auto space-y-2 flex-1">
              {lessons.length === 0 && <p className="text-indigo-500 text-sm text-center py-4">No tenés lecciones disponibles</p>}
              {lessons.map((l) => (
            ...
            <button onClick={() => setDuelModal(null)} className="mt-4 text-indigo-400 hover:text-white text-sm transition text-center">
              Cancelar
            </button>
          </div>
        </div>
...
function FriendCard({ user, onProfile, onRemove, onDuel }) {
  return (
    <div className="bg-indigo-900 border border-indigo-700 rounded-2xl p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0" onClick={onProfile} role="button" style={{cursor:"pointer"}}>
      ...
      <button onClick={onDuel} className="bg-violet-500 hover:bg-violet-400 text-white font-bold px-3 py-1.5 rounded-xl text-sm transition active:scale-95">⚔️</button>
      <button onClick={onRemove} className="text-indigo-600 hover:text-red-400 text-xs transition">Eliminar</button>
    </div>
  );
}
...
function RequestCard({ request, onAccept, onReject }) {
  ...
  return (
    <div className="bg-indigo-900 border border-indigo-700 rounded-2xl p-4 flex items-center gap-4">
      <Avatar src={u.avatar} name={u.displayName || u.username} size="md" className="flex-shrink-0" />
      <div className="flex-1">
