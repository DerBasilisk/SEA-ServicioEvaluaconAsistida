...
export default function Friends() {
  ...
  const [tab, setTab] = useState("friends"); // friends | search | leaderboard | requests
  ...
  const handleSendRequest = async (username) => {
    try {
      ...
      handleSearch(searchQuery); // refrescar resultados
    } catch (err) {
...
function FriendCard({ user, onProfile, onRemove }) {
  return (
  ...
  return (
    <div className="bg-indigo-900 border border-indigo-700 rounded-2xl p-4 flex items-center gap-4">
      ...
      <div className="flex gap-2">
        <button onClick={onAccept} className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold px-3 py-1.5 rounded-xl text-sm transition">
          ✓
        </button>
        <button onClick={onReject} className="bg-red-500/20 hover:bg-red-500/40 text-red-400 font-bold px-3 py-1.5 rounded-xl text-sm transition">
          ✕
        </button>
      </div>
    </div>
    ...
    <div className="bg-indigo-900 border border-indigo-700 rounded-2xl p-4 flex items-center gap-4">
      ...
      {user.friendStatus ? (
        ...
        <button
          ...
          className="bg-violet-500 hover:bg-violet-400 text-white font-bold px-3 py-1.5 rounded-xl text-sm transition active:scale-95"
        >
          + Agregar
        </button>
      )}
